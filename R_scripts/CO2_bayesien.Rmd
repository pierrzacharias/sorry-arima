...
plot(as.mcmc(bsts.reg$sigma.trend.slope[-(1:200)]))

# affichage de la distribution à postériori des résidus du modèle 
PlotBstsResiduals(bsts.reg, burn = SuggestBurn(0.2, bsts.reg), style = "dynamic" )
acf(bsts.reg$sigma.trend.slope[-(1:200)],lag.max = 50)
...
one.step.error = colMeans(bsts.reg$one.step.prediction.errors)
MAPE=mean(abs(one.step.error)/CO2$output.co2)
MAPE
